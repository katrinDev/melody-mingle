...
export function expStringCalculater(experience: number) {
	if (experience) {
		const lastNumber = Number(experience.toString().slice(-1));
		const yearsName =
			lastNumber === 1 ? 'год' : lastNumber < 5 ? 'года' : 'лет';
		return yearsName;
	}
	return experience;
}
...
function Profile() {
	...
	if ((loadingMusician && id) || loadingProjects || !musicianStore.musician) {
					...
					<Stack direction="row" spacing={3} sx={{ display: 'flex', mb: 0.5 }}>
						...
						<Stack spacing={3} sx={{ flexGrow: 1 }}>
							<div>
								...
								<Typography>
									{musician.mainRole} - {musician.experience}{' '}
									{expStringCalculater(musician.experience)} опыта
								</Typography>
							</div>

...
export default class MusicianStore {
	...
	async fetchAllMusicians(snackbarStore: SnackbarPropsStore, isAdmin: boolean) {
		try {
			let response;
			if (isAdmin) {
				response = await MusicianService.getAllMusicians();
			} else {
				response = await MusicianService.getAllOtherMusicians();
			}
			this.setAllMusicians(response.data);
		} catch (error) {
